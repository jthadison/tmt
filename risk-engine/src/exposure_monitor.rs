...
impl ExposureMonitor {
    ...
    async fn calculate_concentration_risk(&self, positions: &[Position]) -> Result<ConcentrationRisk> {
        ...
        let hhi: Decimal = symbol_exposures.values()
            .map(|&exposure| {
        ...
        let largest_position_percentage = symbol_exposures.values()
    ...
    async fn calculate_diversification_score(&self, positions: &[Position]) -> Result<Decimal> {
        ...
        let unique_symbols: std::collections::HashSet<_> = positions.iter()
            .map(|p| &p.symbol)
            .collect();
    ...
    pub async fn get_exposure_by_account(&self, account_id: AccountId) -> Result<AccountExposure> {
        ...
        for position in &positions {
            ...
            *symbol_exposure.entry(position.symbol.clone()).or_insert(dec!(0)) += exposure;
        }
...
impl CurrencyExposureCalculator {
    pub async fn calculate_net_exposure(&self, positions: &[Position]) -> Result<HashMap<String, Decimal>> {
...
impl ExposureLimits {
    ...
    pub async fn get_pair_limit(&self, pair: &str) -> Option<Decimal> {
        self.pair_limits.get(pair).map(|limit| *limit).or(Some(self.default_pair_limit))
    }
    
    pub async fn get_currency_limit(&self, currency: &str) -> Option<Decimal> {
        self.currency_limits.get(currency).map(|limit| *limit).or(Some(self.default_currency_limit))
    }
}
