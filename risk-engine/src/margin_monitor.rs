...
impl MarginMonitor {
    ...
    pub async fn start_monitoring(&self) -> Result<()> {
        let mut ticker = interval(Duration::from_secs(self.margin_thresholds.monitoring_interval_secs));
        info!("Started margin monitoring with {}-second intervals", self.margin_thresholds.monitoring_interval_secs);
    ...
    async fn check_margin_thresholds(&self, account: &Account, margin_info: &MarginInfo) -> Result<()> {
        ...
        if margin_info.margin_level <= self.margin_thresholds.stop_out_level {
            self.margin_alerts.send_emergency_alert(MarginAlert {
                account_id: account.id,
            ...
            self.trigger_emergency_stop_out(account.id, margin_info).await?;
        }
        
        Ok(())
    }
    
    async fn trigger_margin_protection(&self, account_id: AccountId, margin_info: &MarginInfo) -> Result<()> {
        info!(
            "Triggering margin protection for account {} at {:.2}% margin level",
            account_id, margin_info.margin_level
        );
        
        self.margin_protection.protect_account(account_id, margin_info).await?;
        
        Ok(())
    }
    
    async fn trigger_emergency_stop_out(&self, account_id: AccountId, margin_info: &MarginInfo) -> Result<()> {
        warn!(
            "EMERGENCY STOP OUT: Account {} at {:.2}% margin level - closing all positions",
            account_id, margin_info.margin_level
        );
        
        self.margin_protection.emergency_stop_out(account_id, margin_info).await?;
        
        Ok(())
    }
    
    async fn update_margin_cache(&self, account_id: &AccountId, margin_info: &MarginInfo) -> Result<()> {
    ...
    async fn publish_margin_update(&self, margin_info: &MarginInfo) -> Result<()> {
        let _update = RiskUpdate {
            ...
            previous_value: self.margin_cache.get(&margin_info.account_id)
                .map(|prev| prev.margin_level),
            threshold_status: self.determine_threshold_status(margin_info.margin_level),
            timestamp: Utc::now(),
        };
        
        info!("Margin update for account {}: {:.2}%", margin_info.account_id, margin_info.margin_level);
    ...
    pub async fn get_margin_requirements(&self, account_id: AccountId) -> Result<MarginRequirements> {
        let margin_info = self.margin_cache.get(&account_id)
            .map(|m| m.clone())
    ...
    pub async fn simulate_margin_impact(&self, account_id: AccountId, new_position: &ProposedPosition) -> Result<MarginImpact> {
        let current_margin_info = self.margin_cache.get(&account_id)
            .map(|m| m.clone())
            .ok_or_else(|| anyhow::anyhow!("No margin info for account"))?;
        
        let additional_margin = self.margin_calculator
            .calculate_proposed_position_margin(new_position).await?;
...
impl AccountManager {
    ...
    pub async fn get_account_positions(&self, account_id: &AccountId) -> Result<Vec<Position>> {
        Ok(self.account_positions.get(account_id)
    ...
    pub async fn add_position(&self, position: Position) {
        self.account_positions.entry(position.account_id)
...
impl MarginCalculator {
    ...
    pub async fn calculate_position_margin(&self, position: &Position) -> Result<Decimal> {
        let leverage = self.leverage_map.get(&position.symbol)
            .map(|l| *l)
    ...
    pub async fn calculate_proposed_position_margin(&self, proposed: &ProposedPosition) -> Result<Decimal> {
        let leverage = self.leverage_map.get(&proposed.symbol)
            .map(|l| *l)
            .unwrap_or(self.default_leverage);
...
impl MarginAlertManager {
    ...
    async fn store_alert(&self, alert: MarginAlert) -> Result<()> {
        self.alerts.entry(alert.account_id)
            .or_insert_with(Vec::new)
            .push(alert);
        Ok(())
    }
...
impl MarginProtectionSystem {
    pub async fn protect_account(&self, account_id: AccountId, margin_info: &MarginInfo) -> Result<()> {
        info!(
    ...
    pub async fn emergency_stop_out(&self, account_id: AccountId, margin_info: &MarginInfo) -> Result<()> {
        warn!(
