...
async fn test_signal_processing_with_eligible_accounts() {
    ...
    for i in 1..=3 {
        ...
        orchestrator.register_account(
            ...
            platform,
            10000.0,
        ).await.expect("Account registration should succeed");
    }
...
async fn test_signal_processing_no_eligible_accounts() {
    ...
    orchestrator.pause_account("account_1").await.expect("Account pause should succeed");
...
async fn test_timing_variance() {
    ...
    let signal = create_test_signal();
    let plan = orchestrator.process_signal(signal).await.expect("Signal processing should succeed");
...
async fn test_account_pause_resume() {
    ...
    let status1 = orchestrator.get_account_status("account_1").await.expect("Account status should be available");
    assert!(status1.is_active);
    
    orchestrator.pause_account("account_1").await.expect("Account pause should succeed");
    let status2 = orchestrator.get_account_status("account_1").await.expect("Account status should be available");
    assert!(!status2.is_active);
    
    orchestrator.resume_account("account_1").await.expect("Account resume should succeed");
    let status3 = orchestrator.get_account_status("account_1").await.expect("Account status should be available");
...
async fn test_execution_history_logging() {
    ...
    let signal = create_test_signal();
    let _ = orchestrator.process_signal(signal).await.expect("Signal processing should succeed");
    
    let history = orchestrator.get_execution_history(10).await;
    assert!(history.len() > 0, "Execution history should contain entries");
...
async fn test_position_size_calculation_with_drawdown() {
    ...
    let signal = create_test_signal();
    let plan1 = orchestrator.process_signal(signal.clone()).await.expect("Signal processing should succeed");
