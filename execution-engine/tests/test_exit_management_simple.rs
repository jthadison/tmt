use execution_engine::execution::exit_management::{
    BreakEvenConfig, ExitManagementIntegration, NewsProtectionConfig, PartialProfitConfig,
    TimeExitConfig, TrailingConfig,
};
...
async fn test_exit_management_system_creation() {
    ...
    assert!(
        exit_management_result.is_ok(),
        "Should create exit management system successfully"
    );
    ...
    assert!(
        exit_management.is_enabled(),
        "Exit management should be enabled by default"
    );
...
async fn test_exit_management_components_creation() {
    ...
    assert!(
        exit_management.is_enabled(),
        "Built system should be enabled"
    );
...
async fn test_exit_management_with_failing_platform() {
    ...
    assert!(
        exit_management_result.is_ok(),
        "Should create system even with failing platform"
    );
...
async fn test_exit_management_adapter_creation() {
    ...
    assert!(
        components_result.is_ok(),
        "Should create exit management components"
    );
    ...
    assert!(
        exit_management.is_enabled(),
        "System built from components should be enabled"
    );
...
async fn test_exit_management_performance() {
    ...
    assert!(
        ...
        "System creation should complete within 1 second"
    );
