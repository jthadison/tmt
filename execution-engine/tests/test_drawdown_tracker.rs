...
use execution_engine::risk::{
    DrawdownTracker, EquityHistoryManager, DrawdownAlertManager, EquityPoint
};
use std::sync::Arc;
...
async fn test_daily_drawdown_calculation() {
    ...
    for (equity, timestamp) in &equity_points {
        ...
        equity_history_manager.history.entry(account_id)
...
async fn test_drawdown_based_position_sizing() {
    ...
    let adjusted_risk = tracker.trigger_drawdown_based_position_sizing(account_id).await.unwrap();
