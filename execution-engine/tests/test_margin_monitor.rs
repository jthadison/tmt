...
use execution_engine::risk::{
    MarginMonitor, Account, AccountManager, MarginCalculator,
    MarginAlertManager, MarginProtectionSystem, Position, PositionType,
    ProposedPosition
};
use std::sync::Arc;
...
async fn test_margin_thresholds() {
    ...
    monitor.check_margin_thresholds(&account, &margin_info).await.unwrap();
...
async fn test_margin_requirements() {
    ...
    let margin_info = monitor.calculate_account_margin(&account).await.unwrap();
    monitor.update_margin_cache(&account_id, &margin_info).await.unwrap();
...
async fn test_margin_impact_simulation() {
    ...
    let impact = monitor.simulate_margin_impact(account_id, &proposed_position).await.unwrap();
