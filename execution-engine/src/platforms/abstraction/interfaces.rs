...
pub trait ITradingPlatform: Send + Sync {
    ...
    /// Order management
    async fn place_order(&self, order: UnifiedOrder)
...
pub trait IOrderManager: Send + Sync {
    ...
    async fn calculate_margin_requirement(
        &self,
        order: &UnifiedOrder,
    ) -> Result<rust_decimal::Decimal, PlatformError>;
    async fn estimate_commission(
        &self,
        order: &UnifiedOrder,
    ) -> Result<rust_decimal::Decimal, PlatformError>;
    async fn get_order_book(
        ...
        symbol: &str,
        depth: Option<u32>,
    ) -> Result<OrderBook, PlatformError>;
}
...
pub trait IPositionManager: Send + Sync {
    ...
    ) -> Result<Vec<PositionSnapshot>, PlatformError>;
