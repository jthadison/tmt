...
impl FIXMessage {
    pub fn parse(raw_message: &str) -> Result<Self> {
        ...
        for part in parts {
            ...
            let tag: u32 = field_parts[0].parse()
                .map_err(|_| DXTradeError::FixMessageError(
                    format!("Invalid tag: {}", field_parts[0])
                ))?;
    ...
    pub fn validate_checksum(&self) -> bool {
        ...
        if let Some(pos) = checksum_pos {
            ...
            let expected_checksum = message_without_checksum.as_bytes()
                .iter()
                .map(|&b| b as u32)
                .sum::<u32>() % 256;
    ...
    pub fn create_heartbeat(sender_comp_id: String, target_comp_id: String, msg_seq_num: u32) -> Result<Self> {
        FIXMessageBuilder::new(sender_comp_id, target_comp_id, msg_seq_num)
            .build(MessageType::Heartbeat)
    }
    
    pub fn create_test_request(sender_comp_id: String, target_comp_id: String, msg_seq_num: u32, test_req_id: String) -> Result<Self> {
        FIXMessageBuilder::new(sender_comp_id, target_comp_id, msg_seq_num)
            .with_field(112, test_req_id) // TestReqID
            .build(MessageType::TestRequest)
    }
    
    pub fn create_logon(sender_comp_id: String, target_comp_id: String, msg_seq_num: u32, heartbeat_interval: u32, reset_seq_num: bool) -> Result<Self> {
    ...
    pub fn create_logout(sender_comp_id: String, target_comp_id: String, msg_seq_num: u32, text: Option<String>) -> Result<Self> {
        let mut builder = FIXMessageBuilder::new(sender_comp_id, target_comp_id, msg_seq_num);
