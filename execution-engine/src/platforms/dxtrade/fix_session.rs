...
impl FIXSession {
    ...
    pub async fn send_message(&self, message: FIXMessage) -> Result<()> {
        if !self.is_active.load(Ordering::SeqCst) {
            return Err(DXTradeError::FixSessionError(
...

        {
            ...
            if let Some(ref mut stream) = connection_guard.as_mut() {
                stream
                    .write_all(message.raw_message.as_bytes())
                    .await
                    .map_err(|e| {
                        DXTradeError::FixSessionError(format!("Failed to send message: {}", e))
                    })?;
            } else {
                return Err(DXTradeError::FixSessionError(
    ...
    async fn handle_incoming_message(&self, raw_message: &str) -> Result<()> {
        ...
        if received_seq != expected_seq && !message.is_admin_message() {
            tracing::warn!(
                "Sequence number gap detected. Expected: {}, Received: {}",
                expected_seq,
                received_seq
            );
    ...
    async fn handle_resend_request(&self, message: &FIXMessage) -> Result<()> {
        ...
        tracing::info!(
            "Handling resend request for sequences {} to {}",
            begin_seq_no,
            end_seq_no
        );
    ...
    async fn handle_sequence_gap(&self, expected: u32, received: u32) -> Result<()> {
        if received > expected {
            tracing::info!(
                "Sending resend request for sequences {} to {}",
                expected,
                received - 1
            );
    ...
    async fn heartbeat_loop(&self) -> Result<()> {
        ...
        let test_request_delay =
            Duration::from_secs(self.config.connection.test_request_delay_s as u64);
...
impl SessionHandles {
    async fn message_processing_loop(&self) -> Result<()> {
        ...
        let connection = self
            ...
            .upgrade()
            .ok_or_else(|| DXTradeError::FixSessionError("Session was dropped".to_string()))?;
        let _next_seq_num_in = self
            .next_seq_num_in
            .upgrade()
            .ok_or_else(|| DXTradeError::FixSessionError("Session was dropped".to_string()))?;
        let _last_heartbeat_received = self
    ...
    async fn heartbeat_loop(&self) -> Result<()> {
        ...
        let config = self
            ...
            .upgrade()
            .ok_or_else(|| DXTradeError::FixSessionError("Session was dropped".to_string()))?;
        let next_seq_num_out = self
            .next_seq_num_out
            .upgrade()
            .ok_or_else(|| DXTradeError::FixSessionError("Session was dropped".to_string()))?;
        let last_heartbeat_sent = self
            .last_heartbeat_sent
            .upgrade()
            .ok_or_else(|| DXTradeError::FixSessionError("Session was dropped".to_string()))?;
        let last_heartbeat_received = self
            .last_heartbeat_received
            .upgrade()
            .ok_or_else(|| DXTradeError::FixSessionError("Session was dropped".to_string()))?;
