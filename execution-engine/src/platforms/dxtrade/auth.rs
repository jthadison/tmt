...
impl DXTradeAuth {
    pub fn new(config: &DXTradeConfig) -> Result<Self> {
        ...
        if !std::path::Path::new(&ssl_cert_path).exists() {
            return Err(DXTradeError::AuthenticationError(format!(
    ...
    pub fn encrypt_credentials(&mut self, encryption_key: &[u8; 32]) -> Result<()> {
        ...
        let mut credentials_json = serde_json::to_string(&credentials).map_err(|e| {
            DXTradeError::AuthenticationError(format!("Failed to serialize credentials: {}", e))
        })?;

        let cipher = Aes256Gcm::new_from_slice(encryption_key).map_err(|e| {
            DXTradeError::AuthenticationError(format!("Failed to create cipher: {}", e))
        })?;
        ...
        let encrypted_data = cipher
            ...
            .map_err(|e| {
                DXTradeError::AuthenticationError(format!("Failed to encrypt credentials: {}", e))
            })?;
    ...
    pub fn decrypt_credentials(&self, encryption_key: &[u8; 32]) -> Result<EncryptedCredentials> {
        let encrypted_data = self.encrypted_credentials.as_ref().ok_or_else(|| {
            DXTradeError::AuthenticationError("No encrypted credentials available".to_string())
        })?;
        ...
        let mut encrypted_bytes =
            general_purpose::STANDARD
                ...
                .map_err(|e| {
                    DXTradeError::AuthenticationError(format!(
                        "Failed to decode encrypted credentials: {}",
                        e
                    ))
                })?;
        ...
        let mut decrypted_data = cipher
            ...
            .map_err(|e| {
                DXTradeError::AuthenticationError(format!("Failed to decrypt credentials: {}", e))
            })?;

        // Zeroize encrypted data
        encrypted_bytes.zeroize();

        let mut credentials_json = String::from_utf8(decrypted_data.clone()).map_err(|e| {
            DXTradeError::AuthenticationError(format!(
                "Invalid UTF-8 in decrypted credentials: {}",
                e
            ))
        })?;
        ...
        let credentials = serde_json::from_str(&credentials_json).map_err(|e| {
            DXTradeError::AuthenticationError(format!("Failed to deserialize credentials: {}", e))
        })?;
    ...
    pub fn load_ssl_certificate(&self) -> Result<Vec<u8>> {
        fs::read(&self.ssl_cert_path).map_err(|e| {
            DXTradeError::AuthenticationError(format!("Failed to read SSL certificate: {}", e))
        })
    }

    pub fn load_ssl_key(&self) -> Result<Vec<u8>> {
        fs::read(&self.ssl_key_path).map_err(|e| {
            DXTradeError::AuthenticationError(format!("Failed to read SSL private key: {}", e))
        })
    }
