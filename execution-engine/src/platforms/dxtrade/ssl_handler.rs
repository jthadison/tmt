...
impl SslHandler {
    ...
    fn load_identity(config: &SslConfig) -> Result<Identity> {
        ...
        let cert_data = fs::read(&config.cert_file_path).map_err(|e| {
            DXTradeError::SslAuthenticationFailed(format!("Failed to read certificate file: {}", e))
        })?;

        let key_data = fs::read(&config.key_file_path).map_err(|e| {
            DXTradeError::SslAuthenticationFailed(format!("Failed to read key file: {}", e))
        })?;
    ...
    fn load_ca_certificate(ca_path: &str) -> Result<Certificate> {
        ...
        Certificate::from_pem(ca_pem.as_bytes()).map_err(|e| {
            DXTradeError::SslAuthenticationFailed(format!("Failed to parse CA certificate: {}", e))
        })
    }
    ...
    pub async fn connect_to_server(
        ...
        let tcp_stream = TcpStream::connect(format!("{}:{}", hostname, port))
            .await
            .map_err(|e| {
                DXTradeError::ConnectionError(format!(
        ...
        let tls_stream = tls_connector
            ...
            .await
            .map_err(|e| {
                DXTradeError::SslAuthenticationFailed(format!("TLS handshake failed: {}", e))
            })?;
    ...
    pub fn create_rustls_config(&self) -> Result<Arc<ClientConfig>> {
            ...
            .map_err(|e| DXTradeError::TlsError(format!("Failed to load native certs: {}", e)))?
        {
            root_store.add(&rustls::Certificate(cert.0)).map_err(|e| {
                DXTradeError::TlsError(format!("Failed to add certificate: {:?}", e))
            })?;
        }
    ...
    pub fn validate_certificate_chain(&self) -> Result<()> {
        let cert_pem = fs::read_to_string(&self.config.cert_file_path).map_err(|e| {
            DXTradeError::SslAuthenticationFailed(format!("Failed to read certificate file: {}", e))
        })?;

        let _cert = Certificate::from_pem(cert_pem.as_bytes()).map_err(|e| {
