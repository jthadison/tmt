...
mod tests {
    ...
    fn test_config_validation_empty_sender_comp_id() {
        ...
        assert!(result.unwrap_err().to_string().contains("sender_comp_id cannot be empty"));
    }
    ...
    fn test_config_validation_invalid_heartbeat_interval() {
        ...
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("heartbeat_interval_s must be at least 10 seconds"));
    }
    ...
    fn test_dxtrade_environment_urls() {
        ...
        assert_eq!(staging.rest_base_url(), "https://api-staging.dxtrade.com/v2");
    }
