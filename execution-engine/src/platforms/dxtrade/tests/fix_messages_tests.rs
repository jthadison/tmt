...
mod tests {
    ...
    fn test_fix_message_field_extraction() {
        ...
        assert_eq!(
            message.get_field_as_decimal(44).unwrap().to_string(),
            "100.50"
        );
    ...
    fn test_heartbeat_creation() {
        let heartbeat =
            FIXMessage::create_heartbeat("TEST_SENDER".to_string(), "TEST_TARGET".to_string(), 42)
                .unwrap();
