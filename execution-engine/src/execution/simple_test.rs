...
mod tests {
    ...
    async fn test_orchestrator_creation() {
        ...
        assert_eq!(history.len(), 0, "New orchestrator should have empty history");
        ...
        assert_eq!(result.unwrap_err(), "No eligible accounts for signal execution");
    }
    ...
    async fn test_correlation_matrix_operations() {
        ...
        orchestrator.update_correlation_matrix("account1", "account2", 0.85).await;
        orchestrator.update_correlation_matrix("account1", "account3", 0.45).await;
        orchestrator.update_correlation_matrix("account2", "account3", 0.92).await;
        ...
        assert!(resume_result.is_err(), "Should fail for nonexistent account");
        
        let status = orchestrator.get_account_status("nonexistent_account").await;
        assert!(status.is_none(), "Should return None for nonexistent account");
    }
