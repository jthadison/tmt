...
impl DrawdownTracker {
    ...
    async fn calculate_daily_drawdown(&self, equity_history: &[EquityPoint]) -> Result<DrawdownData> {
        ...
        let peak_equity = today_points.iter()
            .map(|p| p.equity)
            .max()
            .unwrap_or(dec!(0));
    ...
    async fn calculate_weekly_drawdown(&self, equity_history: &[EquityPoint]) -> Result<DrawdownData> {
        let one_week_ago = Utc::now() - Duration::days(7);
        let week_points: Vec<_> = equity_history.iter()
        ...
        let peak_time = week_points.iter()
    ...
    async fn calculate_underwater_period(&self, equity_history: &[EquityPoint]) -> Result<Duration> {
        if equity_history.is_empty() {
    ...
    async fn check_drawdown_alerts(&self, account_id: AccountId, metrics: &DrawdownMetrics) -> Result<()> {
        if metrics.daily_drawdown.percentage > self.thresholds.daily_threshold {
            self.drawdown_alerts.send_alert(DrawdownAlert {
                account_id,
                ...
                message: format!(
                    ...
                    metrics.daily_drawdown.percentage,
                    self.thresholds.daily_threshold
                ),
                timestamp: Utc::now(),
            }).await?;
        }
        ...
        if metrics.weekly_drawdown.percentage > self.thresholds.weekly_threshold {
            self.drawdown_alerts.send_alert(DrawdownAlert {
                ...
                message: format!(
                    ...
                    metrics.weekly_drawdown.percentage,
                    self.thresholds.weekly_threshold
                ),
                timestamp: Utc::now(),
            }).await?;
        }
        ...
        if metrics.maximum_drawdown.percentage > self.thresholds.max_threshold {
            self.drawdown_alerts.send_alert(DrawdownAlert {
                ...
                message: format!(
                    ...
                    metrics.maximum_drawdown.percentage,
...
impl EquityHistoryManager {
    ...
    pub async fn get_history(&self, account_id: AccountId, duration: Duration) -> Result<Vec<EquityPoint>> {
        let cutoff_time = Utc::now() - duration;
        
        let history = self.history.get(&account_id)
            .map(|h| h.clone())
            .unwrap_or_default();
    ...
    pub async fn record_equity(&self, account_id: AccountId, equity: Decimal, balance: Decimal) -> Result<()> {
