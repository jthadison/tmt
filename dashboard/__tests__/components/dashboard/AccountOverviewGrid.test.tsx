...
jest.mock('@/components/ui/LoadingSkeleton', () => ({
  CardSkeleton: jest.fn(({ className }) => (
    <div data-testid="card-skeleton" className={className}></div>
  ))
}))
...
describe('AccountOverviewGrid', () => {
  ...
  it('filters accounts by search term', async () => {
    ...
    await user.type(searchInput, 'FTMO')
    
    // Should show only FTMO account
    expect(screen.getByTestId('account-card-account-1')).toBeInTheDocument()
    expect(screen.queryByTestId('account-card-account-2')).not.toBeInTheDocument()
  ...
  it('filters accounts by status', async () => {
    ...
    const dangerCheckbox = screen.getByLabelText('Danger')
    await user.click(dangerCheckbox)
    
    // Should show only danger account
    expect(screen.queryByTestId('account-card-account-1')).not.toBeInTheDocument()
    expect(screen.getByTestId('account-card-account-2')).toBeInTheDocument()
    expect(screen.getByText('1 of 2 accounts')).toBeInTheDocument()
  })

  it('sorts accounts by name ascending', async () => {
    const user = userEvent.setup()
    render(<AccountOverviewGrid {...defaultProps} />)
    
    const sortSelect = screen.getByDisplayValue('Name A-Z')
    await user.selectOptions(sortSelect, 'accountName-asc')
    
    // AccountCard mock should be called with accounts in correct order
    const { AccountCard } = require('@/components/dashboard/AccountCard')
    expect(AccountCard).toHaveBeenCalledWith(
      expect.objectContaining({ account: mockAccounts[0] }),
      expect.anything()
    )
  })
  ...
  it('sorts accounts by balance descending', async () => {
    ...
    const sortSelect = screen.getByDisplayValue('Name A-Z')
    await user.selectOptions(sortSelect, 'balance-desc')
    
    // Higher balance account should come first
    const { AccountCard } = require('@/components/dashboard/AccountCard')
    expect(AccountCard).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ account: mockAccounts[0] }), // $100k account
      expect.anything()
    )
  })
  ...
  it('calls onAccountClick when account card is clicked', async () => {
    const user = userEvent.setup()
    render(<AccountOverviewGrid {...defaultProps} />)
    
    const accountCard = screen.getByTestId('account-card-account-1')
  ...
  it('shows no accounts found message when all filtered out', async () => {
    const user = userEvent.setup()
    render(<AccountOverviewGrid {...defaultProps} />)
