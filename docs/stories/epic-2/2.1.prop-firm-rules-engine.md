...
- [ ] Task 1: Create Compliance Agent service structure (AC: 1)
  ...
  - [ ] Implement prop_firm_configs.py with FTMO, FundedNext, MyForexFunds rules
  - [ ] Configure agent to consume Kafka events for trade validation
- [ ] Task 2: Implement prop firm rule configurations (AC: 1)
  - [ ] Define FTMO rules: 5% daily loss, 10% max loss, news trading restrictions
  - [ ] Define FundedNext rules: 4% daily loss, 8% max loss, hold time requirements
  - [ ] Define MyForexFunds rules: 5% daily loss, 12% max loss, lot size limits
